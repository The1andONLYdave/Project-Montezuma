...
add(virtualPad);
		...
		_text3 = new FlxText(FlxG.width * 2 / 5, FlxG.height * 2 / 3, 150, "Play again");
	...
	var _right:Bool = false;
		...
		_up = _up || WinningState.virtualPad.buttonUp.status == FlxButton.PRESSED;
		_down = _down || WinningState.virtualPad.buttonDown.status == FlxButton.PRESSED;
		_left  = _left || WinningState.virtualPad.buttonLeft.status == FlxButton.PRESSED;
		_right = _right || WinningState.virtualPad.buttonA.status == FlxButton.PRESSED;

		
		if ((WinningState.virtualPad.buttonUp.status == FlxButton.PRESSED) && moveIt== false)
		{
		...
		if ((WinningState.virtualPad.buttonUp.status == FlxButton.NORMAL)&& (WinningState.virtualPad.buttonDown.status == FlxButton.NORMAL)){
        moveIt = false;
		}
		
		if ((FlxG.keys.justPressed.DOWN || WinningState.virtualPad.buttonDown.status == FlxButton.PRESSED)&& moveIt== false)
		{
			_option = (_option + OPTIONS + 1) % OPTIONS;
			FlxG.sound.play("assets/sounds/menu" + Reg.SoundExtension, 1, false);
			moveIt=true;
		...
		if (FlxG.keys.justPressed.RIGHT || WinningState.virtualPad.buttonA.status == FlxButton.PRESSED)
		{
			switch (_option) 
			{
				case 0:
